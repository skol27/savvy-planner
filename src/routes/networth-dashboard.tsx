...
export const Route = createFileRoute("/networth-dashboard")({
  head: () => ({
    meta: [
      { title: "Net Worth Dashboard — Ledger" },
      {
        name: "description",
        content: "Net worth, allocation, IRR/ECR, contributions, and goal analysis.",
      },
    ],
  }),
...
type NWDetail = "Categories" | "Positions";
type NWSortBy = "Value" | "Start" | "Contrib" | "Gain" | "IRR";
type NWHorizon = "All" | "ST" | "LT";
type NWType = "Assets" | "Liabilities";
type NWDetailRow = {
  name: string;
  category: string;
  current: number;
...
const detailSortKeys = {
  ...
  NWSortBy,
  keyof Pick<NWDetailRow, "current" | "start" | "contrib" | "gain" | "irr">
>;

function NWDashboard() {
  const { nwPositions, transactions, settings, goals, setGoals, assetCats, liabCats } =
    useFinance();
  const months = useMemo(
    () => [...new Set(nwPositions.flatMap((p) => Object.keys(p.balances)))].sort(),
    [nwPositions],
  );
  const latest =
    latestTrackedMonth(nwPositions, settings.latestTrackedMode) ||
    months[months.length - 1] ||
    "2026-02";
  const [from, setFrom] = useState(months[0] || "2026-02");
  const [to, setTo] = useState(latest);
  const [detail, setDetail] = useState<NWDetail>("Positions");
  const [sortBy, setSortBy] = useState<NWSortBy>("Value");
  const [horizon, setHorizon] = useState<NWHorizon>("All");
  const [type, setType] = useState<NWType>("Assets");
  ...
  const contribs = transactions
    .filter((t) => {
      ...
      return mk >= from && mk <= to && (t.budgetType === "Savings" || t.budgetType === "Debt");
    })
  ...
  const series = months.map((m) => {
    ...
    return {
      ...
      assets: t.assets,
      liab: t.liab,
      contrib: contribs * (months.indexOf(m) / Math.max(1, months.length - 1)),
    };
  });

  const allocAssets = nwPositions
    .filter((p) => p.type === "Asset")
    .map((p) => ({ name: p.name, value: p.balances[to] || 0 }))
    .filter((x) => x.value > 0);
  const allocLiab = nwPositions
    .filter((p) => p.type === "Liability")
    .map((p) => ({ name: p.name, value: p.balances[to] || 0 }))
    .filter((x) => x.value > 0);
  ...
  const detailRows = useMemo(() => {
    const items =
      type === "Assets"
        ? nwPositions.filter((p) => p.type === "Asset")
        : nwPositions.filter((p) => p.type === "Liability");
    ...
    const rows: NWDetailRow[] = filtered.map((p) => {
      const category = cats.find((c) => c.id === p.categoryId)?.name || "—";
      const startV = p.balances[from] || 0;
      const endV = p.balances[to] || 0;
      const contrib = transactions
        .filter(
          (t) => t.account === p.name && t.date.slice(0, 7) >= from && t.date.slice(0, 7) <= to,
        )
        .reduce((a, t) => a + t.amount, 0);
      const g = endV - startV - contrib;
      const r = irrApprox(Math.abs(startV), Math.abs(endV), contrib, periods);
      return { name: p.name, category, current: endV, start: startV, contrib, gain: g, irr: r };
    });
    const grouped =
      detail === "Positions"
        ? rows
        : Array.from(
            rows
              .reduce((map, row) => {
                ...
                cur.current += row.current;
                cur.start += row.start;
                cur.contrib += row.contrib;
                cur.gain += row.gain;
            ...
            ...row,
            irr: irrApprox(Math.abs(row.start), Math.abs(row.current), row.contrib, periods),
          }));
    grouped.sort((a, b) => b[detailSortKeys[sortBy]] - a[detailSortKeys[sortBy]]);
    return grouped;
  }, [
    nwPositions,
    type,
    ...
    <div className="flex flex-col">
      ...
      <div className="p-6 space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          <KpiCard
            label="IRR"
            value={settings.irrEcr ? `${(irr * 100).toFixed(1)}%` : "—"}
            hint={settings.irrEcr ? "" : "Disabled in settings"}
          />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { title: "Asset Allocation", data: allocAssets },
            { title: "Liability Allocation", data: allocLiab },
          ].map((d, i) => (
        ...
        <div className="rounded-lg border bg-card overflow-hidden">
          <div className="flex flex-wrap items-center gap-2 p-3 border-b">
            ...
            <ToggleGroup
              ...
              size="sm"
              value={type}
              onValueChange={(v) => v && setType(v as NWType)}
            >
              <ToggleGroupItem value="Assets">Assets</ToggleGroupItem>
              <ToggleGroupItem value="Liabilities">Liabilities</ToggleGroupItem>
            </ToggleGroup>
            <ToggleGroup
              type="single"
              size="sm"
              value={horizon}
              onValueChange={(v) => v && setHorizon(v as NWHorizon)}
            >
              <ToggleGroupItem value="All">All</ToggleGroupItem>
              <ToggleGroupItem value="ST">ST</ToggleGroupItem>
            ...
            <ToggleGroup
              ...
              size="sm"
              value={detail}
              onValueChange={(v) => v && setDetail(v as NWDetail)}
            >
              <ToggleGroupItem value="Categories">Cat</ToggleGroupItem>
              <ToggleGroupItem value="Positions">Pos</ToggleGroupItem>
            </ToggleGroup>
            <Select value={sortBy} onValueChange={(v) => setSortBy(v as NWSortBy)}>
              <SelectTrigger className="h-8 w-28">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {["Value", "Start", "Contrib", "Gain", "IRR"].map((s) => (
            ...
            <div className="border-r">
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={detailRows.slice(0, 8)} layout="vertical">
                  ...
                  <Bar
                    dataKey="current"
                    fill={type === "Assets" ? "var(--color-pos)" : "var(--color-neg)"}
                    radius={[0, 3, 3, 0]}
                  />
                </BarChart>
              </ResponsiveContainer>
              ...
              <tbody>
                {detailRows.map((r) => (
                  <tr key={r.name} className="border-t hover:bg-muted/20">
                    ...
                    <td className="px-2 text-right num text-muted-foreground">
                      {totalCurrent ? `${((r.current / totalCurrent) * 100).toFixed(0)}%` : "—"}
                    </td>
                    ...
                    <td className="px-2 text-right num">
                      {settings.irrEcr ? `${(r.irr * 100).toFixed(1)}%` : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  <tr key={g.id} className="border-t">
                    ...
                    <td className="text-center">
                      <Switch
                        ...
                        onCheckedChange={(v) =>
                          setGoals((prev) =>
                            prev.map((x) => (x.id === g.id ? { ...x, show: v } : x)),
                          )
                        }
                      />
                    </td>
          ...
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs">
              <div>
                <label className="text-muted-foreground">Runway (months)</label>
                <Input
                  type="number"
                  value={runway}
              ...
              <div>
                <label className="text-muted-foreground">Monthly return %</label>
                <Input
                  type="number"
                  step="0.1"
                  value={monthlyReturn}
              ...
              <div>
                <label className="text-muted-foreground">Monthly withdraw</label>
                <Input
                  type="number"
            ...
            <div className="grid grid-cols-3 gap-3 mt-4">
              <KpiCard
                label={`After ${runway}m`}
                value={fmt(
                  end.net * Math.pow(1 + monthlyReturn / 100, runway) - monthlyWithdraw * runway,
                )}
              />
              <KpiCard label="Sustainable / mo" value={fmt((end.net * monthlyReturn) / 100)} />
              <KpiCard
                label="Months to zero"
                value={Math.max(0, Math.round(end.net / monthlyWithdraw))}
              />
            </div>
          </div>
        </div>
