              ...
              <div className="divide-y">
                {list.map(c => (
                  <div key={c.id} className="flex items-center gap-2 px-3 py-1.5">
                    {edit ? (
                      <Input value={c.name} onChange={e => setBudgetCats(prev => prev.map(x => x.id === c.id ? { ...x, name: e.target.value } : x))} className="h-7 text-sm px-2"/>
                    ) : (
                      <span className="flex-1 text-sm px-2 py-1">{c.name}</span>
                    )}
                    {edit && (
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border bg-card">
        <div className="flex items-center justify-between p-3 border-b">
          ...
          {edit && <Button size="sm" variant="outline" onClick={() => setAssetCats(p => [...p, { id: uid(), name: "New Asset", horizon: "ST", cash: false, available: false }])}><Plus className="h-3.5 w-3.5"/>Add</Button>}
        </div>
          ...
          <tbody>
            {assetCats.map(c => (
              <tr key={c.id} className="border-b last:border-0 hover:bg-muted/30">
                <td className="px-3 py-1">{edit ? <Input value={c.name} onChange={e => setAssetCats(p => p.map(x => x.id===c.id ? {...x, name: e.target.value} : x))} className="h-7 px-2"/> : <span className="px-2">{c.name}</span>}</td>
                <td className="px-2"><Badge variant={c.horizon === "ST" ? "secondary" : "default"} onClick={() => edit && setAssetCats(p => p.map(x => x.id===c.id ? {...x, horizon: x.horizon === "ST" ? "LT" : "ST"} : x))} className={edit ? "cursor-pointer" : ""}>{c.horizon}</Badge></td>
                <td className="text-center px-2"><Checkbox checked={c.cash} disabled={!edit} onCheckedChange={v => setAssetCats(p => p.map(x => x.id===c.id ? {...x, cash: !!v} : x))}/></td>
                <td className="text-center px-2"><Checkbox checked={c.available} disabled={!edit} onCheckedChange={v => setAssetCats(p => p.map(x => x.id===c.id ? {...x, available: !!v} : x))}/></td>
                <td className="px-2">{edit && <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setAssetCats(p => p.filter(x => x.id !== c.id))}><Trash2 className="h-3.5 w-3.5 text-destructive"/></Button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="rounded-lg border bg-card">
        <div className="flex items-center justify-between p-3 border-b">
          ...
          {edit && <Button size="sm" variant="outline" onClick={() => setLiabCats(p => [...p, { id: uid(), name: "New Liability", horizon: "ST" }])}><Plus className="h-3.5 w-3.5"/>Add</Button>}
        </div>
        <table className="w-full text-sm">
          <thead className="text-xs text-muted-foreground"><tr className="border-b">
            <th className="text-left px-3 py-2 font-medium">Category</th>
          ...
          <tbody>
            {liabCats.map(c => (
              <tr key={c.id} className="border-b last:border-0 hover:bg-muted/30">
                <td className="px-3 py-1">{edit ? <Input value={c.name} onChange={e => setLiabCats(p => p.map(x => x.id===c.id ? {...x, name: e.target.value} : x))} className="h-7 px-2"/> : <span className="px-2">{c.name}</span>}</td>
                <td className="px-2"><Badge variant={c.horizon === "ST" ? "secondary" : "default"} onClick={() => edit && setLiabCats(p => p.map(x => x.id===c.id ? {...x, horizon: x.horizon === "ST" ? "LT" : "ST"} : x))} className={edit ? "cursor-pointer" : ""}>{c.horizon}</Badge></td>
                <td className="px-2">{edit && <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => setLiabCats(p => p.filter(x => x.id !== c.id))}><Trash2 className="h-3.5 w-3.5 text-destructive"/></Button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
