...
export const Route = createFileRoute("/networth")({
  head: () => ({
    meta: [
      { title: "Net Worth Tracker — Ledger" },
      { name: "description", content: "Track monthly balances of all assets and liabilities." },
    ],
  }),
  component: NWPage,
});
...
function NWPage() {
  ...
  const baseMonth = (() => {
    const m = settings.startingMonth === 1 ? 12 : settings.startingMonth - 1;
    const y = settings.startingMonth === 1 ? settings.startingYear - 1 : settings.startingYear;
  ...
  const monthStatus = (m: string): { label: string; cls: string } => {
    ...
    const completed =
      nwPositions.length > 0 && nwPositions.every((p) => (p.balances[m] || 0) !== 0);
    if (m < baseMonth)
      return { label: "Ignored", cls: "border-muted-foreground text-muted-foreground" };
    if (m === baseMonth) return { label: "Base", cls: "border-warning text-warning" };
    ...
    if (completed) return { label: "Completed", cls: "border-pos text-pos" };
    return { label: "Tracked", cls: "border-info text-info" };
  };
  ...
  const addPos = (type: "Asset" | "Liability") => {
    ...
    setNwPositions((p) => [
      ...p,
      { id: uid(), type, categoryId: cats[0].id, name: `New ${type}`, balances: {} },
    ]);
  };

  const balanceKey = (id: string, month: string) => `${id}|${month}`;
  const commitBalanceDraft = (id: string, month: string, raw: string) => {
    const trimmed = raw.trim().replace(",", ".");
    const amount = trimmed === "" ? 0 : Number(trimmed);
    if (!Number.isFinite(amount)) return;
    setNwPositions((prev) =>
      prev.map((p) => (p.id === id ? { ...p, balances: { ...p.balances, [month]: amount } } : p)),
    );
    setBalanceDrafts((prev) => {
      const next = { ...prev };
      delete next[balanceKey(id, month)];
      return next;
    });
  };
  const commitBalanceDrafts = () => {
    const drafts = Object.entries(balanceDrafts);
    ...
    setNwPositions((prev) =>
      prev.map((p) => {
        ...
        drafts.forEach(([key, raw]) => {
          ...
          const amount = trimmed === "" ? 0 : Number(trimmed);
          if (!Number.isFinite(amount)) return;
          balances = { ...balances, [month]: amount };
        });
        return balances === p.balances ? p : { ...p, balances };
      }),
    );
    setBalanceDrafts({});
  };
    ...
    return (
      <div className="rounded-lg border bg-card overflow-hidden">
        <div className="flex items-center justify-between p-3 border-b bg-muted/30">
          ...
          {edit && (
            <Button size="sm" variant="outline" onClick={() => addPos(type)}>
          ...
          <table className="w-full text-xs">
            <thead className="bg-muted/20">
              <tr>
                ...
                {visible.map((m) => (
                  <th key={m} className="text-right px-2 py-1.5 font-medium w-24">
                    {m.slice(2)}
                  </th>
                ))}
                <th className="w-8"></th>
            ...
            <tbody>
              {rows.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-t hover:bg-muted/20">
                    ...
                    {visible.map((m) => {
                      const key = balanceKey(p.id, m);
                      const value = balanceDrafts[key] ?? (p.balances[m] || "");
                      return (
                        <td key={m} className="px-1 py-1">
                          {edit ? (
                            <Input
                              ...
                              value={value}
                              onChange={(e) =>
                                setBalanceDrafts((prev) => ({ ...prev, [key]: e.target.value }))
                              }
                              onBlur={() => commitBalanceDraft(p.id, m, String(value))}
                              className="h-7 text-xs num text-right px-1"
                            />
                          ) : (
                            <div className="text-xs num text-right px-1">
                              {p.balances[m] ? fmt(p.balances[m], 0) : "—"}
                            </div>
                          )}
                        </td>
                      );
                    ...
                    <td>
                      {edit && (
                        <Button
                          ...
                          className="h-7 w-7"
                          onClick={() =>
                            setNwPositions((prev) => prev.filter((x) => x.id !== p.id))
                          }
                        >
              ...
              <tr className="border-t bg-muted/20 font-semibold">
                ...
                {visible.map((m) => {
                  ...
                  return (
                    <td key={m} className="px-2 py-1.5 text-right num">
                      {fmt(sum)}
                    </td>
                  );
                })}
  ...
  return (
    <div className="flex flex-col">
      <PageHeader
        ...
        description={`Latest tracked: ${latest || "—"} (${settings.latestTrackedMode} mode)`}
        actions={
          <>
            <Button
              size="sm"
            ...
            <Button size="sm" variant="outline" onClick={() => setFrom(Math.max(0, from - 6))}>
              ← Earlier
            </Button>
            <Button
              size="sm"
              variant="outline"
              onClick={() => setFrom(Math.min(months.length - 6, from + 6))}
            >
              Later →
            </Button>
          </>
      ...
      <div className="p-6 space-y-4">
            ...
            {visible.map((m) => {
              ...
              return (
                <div key={m} className="rounded-md border px-2 py-1 text-xs">
                  ...
                  <Badge variant="outline" className={cn("text-[10px] mt-0.5", s.cls)}>
                    {s.label}
                  </Badge>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-lg border bg-card p-4">
            ...
            <div className="num text-2xl font-semibold text-pos">
              {fmt(latest ? nwTotals(nwPositions, latest).assets : 0)}
            </div>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <div className="text-xs text-muted-foreground uppercase">Total Liabilities</div>
            <div className="num text-2xl font-semibold text-neg">
              {fmt(latest ? nwTotals(nwPositions, latest).liab : 0)}
            </div>
          </div>
          <div className="rounded-lg border bg-card p-4">
            <div className="text-xs text-muted-foreground uppercase">Net Worth</div>
            <div className="num text-2xl font-semibold">
              {fmt(latest ? nwTotals(nwPositions, latest).net : 0)}
            </div>
          </div>
        </div>
      </div>
